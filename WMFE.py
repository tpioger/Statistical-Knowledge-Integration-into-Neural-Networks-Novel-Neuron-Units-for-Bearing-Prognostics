...
for nb_run in range(runs):
    ...
    for i in range(len(bearing)):
        ...
        train_norm, val_norm, train_rul_norm, val_rul_norm = train_test_split(train_norm, train_rul_norm,
                                                                              ...
                                                                              shuffle=True)
        model = ml.model_custom_block_stat(input_shape=window,use_bias=True)
    ...
    if save:
        for j in range(len(prediction_list)):
            with open(f'WMFE_runs/WMFE_prediction_{epochs}_epochs_slicing_{slicing}_bearing_{j+1}_run_{nb_run+1}', 'wb') as f:
